...
6. **Grounding-guard starvation**: a symbol that keeps failing the guard never gets a verdict row, so it never gets the cooldown, and it sits at the head of the `last_quote_at asc` order consuming a slot every week. Tolerable at 10 slots against <20 stale symbols, but log guard failures distinctly so the pattern is diagnosable rather than silent.
