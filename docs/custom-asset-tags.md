...
- Fetch tag definitions and assignments in bulk for the private assets page; reuse that data across cells and dialogs rather than fetching per row. Use ordered `.range()` pages of at most 1000 rows until exhausted for both lists; assignments use `(position_id, tag_id)` ordering. Ownership lookups and bulk removals filter by ID through the URL, so chunk their ID lists (200 per request) instead of paginating results. The local API cap is 1000; increasing `.limit()` does not bypass it.
