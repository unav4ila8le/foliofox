...
- The Tags filter uses **Match all selected tags** (AND), combined with case-insensitive asset-name search. With no tags selected, only the name search applies. Clear filters restores the full list; counts show matching and total assets. Filtering preserves category grouping and sorting.
- **Manage tags**, next to the filter, supports create, rename, recolor, and confirmed deletion. Names are trimmed, required, unique per user ignoring case, and limited to 64 characters. The ten colors are neutral, red, orange, amber, green, teal, blue, indigo, violet, and pink; new tags default to blue. Deleting a tag removes its assignments, including on archived assets, but never deletes assets.
