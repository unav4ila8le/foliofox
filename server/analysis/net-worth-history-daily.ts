...
export async function fetchNetWorthHistoryDaily({
  targetCurrency,
  daysBack = 180,
}: {
  ...
  const marketDateIndices = Array.from(eligibleDateIndices).sort((a, b) => a - b);
  ...
  const marketPositionsMinimal = await toMarketDataPositions(marketEligiblePositions);
